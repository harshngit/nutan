...
const ProductPage = () => {
  ...
  const totalPages = Math.ceil(LargeProductData.length / ITEMS_PER_PAGE);

  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  const visibleProducts = LargeProductData.slice(startIndex, endIndex);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center gap-[32px]">
          {visibleProducts.map((product, index) => (
            <Link href={`/shop/${product.id}`} key={index}>
              <div className="relative lg:w-[285px] group border overflow-hidden">
          ...
          <div className="flex items-center gap-[20px] flex-wrap">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`w-12 h-12 flex items-center justify-center rounded-lg text-sm font-medium transition-colors ${currentPage === page
            ...
            <button
              ...
              className={`px-6 h-12 flex items-center justify-center rounded-lg text-sm font-medium transition-colors ${currentPage === totalPages
